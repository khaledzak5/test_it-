...
from ..models import Department, User, College  # Department يحتوي hod_name, head_user_id (اختياري)
...
def department_create(
    ...
    name: str = Form(...),
    college: str = Form(...),
    hod_name: Optional[str] = Form(None),          # ✅ إدخال يدوي
    head_user_id: Optional[int] = Form(None),      # اختياري إن أردت ربطًا بمستخدم
    ...
    # أدمن الكلية لا يحق له إضافة قسم إلا في كليته
    cu = get_current_user(request, db)
    ...
    dep = Department(
        name=name_n,
        college=college_n,
        hod_name=hod_name_n or None,  # نص يدوي
        head_user_id=head_user.id if head_user else None,  # ربط (اختياري)
        is_active=True
...
def department_update(
    ...
    college: str = Form(...),
    hod_name: Optional[str] = Form(None),          # ✅ إدخال يدوي
    head_user_id: Optional[int] = Form(None),      # اختياري
    is_active: Optional[str] = Form("on"),
):
    dep = db.get(Department, dep_id)
    if not dep:
        raise HTTPException(status_code=404, detail="القسم غير موجود")
    cu = get_current_user(request, db)
    # أدمن الكلية لا يحق له نقل القسم إلى كلية أخرى ولا تعديل قسم خارج كليته
    if cu and cu.is_college_admin and cu.college_admin_college:
...
def department_toggle(
    ...
    dep = db.get(Department, dep_id)
    if dep:
        # أدمن الكلية لا يحق له تبديل حالة قسم خارج كليته
        if request:
            cu = get_current_user(request, db)
            if cu and cu.is_college_admin and cu.college_admin_college and normalize(dep.college) != normalize(cu.college_admin_college):
