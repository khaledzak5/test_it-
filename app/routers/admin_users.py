...
def get_colleges(db: Session) -> List[str]:
    ...
    # جلب الكليات من departments.college (للتوافق مع البيانات القديمة)
    dept_colleges = (
    ...
    return [c for c in all_colleges if c]  # إزالة القيم الفارغة
...
def users_list(
    ...
    # إذا كان أدمن كلية، عرض فقط المستخدمين المرتبطين بكليته
    if current_user and current_user.is_college_admin and current_user.college_admin_college:
        # عرض المستخدمين الذين:
        # 1. هم أدمن نفس الكلية
        # 2. أو رؤساء أقسام في نفس الكلية
        # 3. أو مرتبطين بأقسام في نفس الكلية
        ...
        # جلب رؤساء الأقسام في هذه الكلية
        ...
        # المستخدمين المرتبطين بهذه الكلية
        query = query.filter(
...
def user_create(
    ...
    # لا حاجة لاستخراج يدوي؛ تم تمرير is_doc_f عبر Form

    # قيود الدور حسب المستخدم الحالي
    if cu.is_hod:
        # رئيس القسم يضيف أطباء فقط
        is_admin = False
    ...
    if cu.is_college_admin and not cu.is_admin:
        # أدمن الكلية لا يضيف سوبر أدمن ولا أدمن كلية
        ...
        # إذا أضيف رئيس قسم، يجب أن تكون الكلية هي كلية الأدمن
        if is_hod:
    ...
    if is_college_admin:
        ...
        # لا يمكن أن يكون سوبر أدمن وأدمن كلية في نفس الوقت
        if is_admin:
            is_admin = False
    
    # لا يمكن أن يكون أدمن كلية ورئيس قسم في نفس الوقت
    ...
    # ربط المستخدم بقسم إذا تم اختياره
    ...
    try:
        ...
        # ربط المستخدم بالقسم إذا تم اختياره
        if head_department:
...
def user_update(
    ...
    # معالجة ربط المستخدم بقسم
    # إزالة الربط من جميع الأقسام أولاً
    db.query(Department).filter(Department.head_user_id == user_id).update({"head_user_id": None})
    
    # ربط المستخدم بالقسم الجديد إذا تم اختياره
    if head_user_department_id and head_user_department_id.strip():
        try:
            dept_id = int(head_user_department_id.strip())
            head_department = db.query(Department).filter(Department.id == dept_id).first()
