...
def gi(db: Session, key: str, default: int = 0) -> int:
    ...
    try:
        return S.get_int(db, key, default)  # type: ignore[attr-defined]
    except AttributeError:
        try:
...
def gj(db: Session, key: str, default=None):
    ...
    try:
        return S.get_json(db, key, default)  # type: ignore[attr-defined]
    except AttributeError:
        try:
            raw = gs(db, key, "")
...
def sj(db: Session, key: str, value) -> None:
    ...
    try:
        S.set_json(db, key, value)  # type: ignore[attr-defined]
    except AttributeError:
...
# --------- Cache busting (المهم لتحديث الواجهة مباشرة) ----------
def _bust_settings_cache(db: Session) -> None:
    ...
    """
    # 1) لو الخدمة فيها دالة تفريغ كاش، نستخدمها
    try:
        S.invalidate_cache()  # type: ignore[attr-defined]
    except Exception:
        pass

    # 2) نرفع نسخة الكاش في DB ليكون fallback أكيد
    try:
...
def settings_index(
    ...
    # Colleges behavior (ثابت حسب تصميمك الحالي)
    ctx.update({
...
def settings_save(
    ...
    if t == "general":
        ...
        # لا شيء يُحفظ الآن (المصدر ثابت من الأقسام)
        pass
    ...
    # 👇 أهم سطرين: تفريغ الكاش + تأكيد الكتابة
