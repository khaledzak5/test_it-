...
from ..models import Course, CourseEnrollment, College
...
def verify_page(code: str, request: Request, db: Session = Depends(get_db)):
    ...
    # الحصول على كلية من اسم المزود (provider) أو college_name من البيانات
    college = None
    college_name_from_db = row.get("college_name")
    
    if college_name_from_db:
        # البحث عن الكلية بالاسم
